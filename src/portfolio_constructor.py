...
class PortfolioConstructor:
    ...
    def construct_portfolio(
        ...
        if self.prev_positions is None:
            turnover = 0.0  # no cost for initial entry
        else:
...
def get_sharpe_ratio(returns,periods_per_year = 252):
    """
    Annualised Sharpe = mean(r) / std(r) * sqrt(periods_per_year).
    Returns 0.0 for empty or constant return series.
    """
...
def evaluate_model_per_fold(
    ...
    with torch.no_grad():
        for i in range(len(fold_data.test_idx)):
            ...
            max_drawdown = max(max_drawdown,1.0 - curr_portfolio_value/(min_portfolio_value + 1e-12))  
            ...
            fold_metrics.append(metrics)          # fixed: was append[metrics]
